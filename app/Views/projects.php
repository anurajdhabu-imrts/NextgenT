...
<section class="relative w-full overflow-hidden bg-navy">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24">
        <div class="flex items-center justify-center text-center" data-aos="fade-up">
            ...
            <div class="max-w-3xl">
                <span
                    class="inline-block px-3 py-1 bg-teal-accent/10 text-teal-accent rounded-full text-xs font-bold uppercase tracking-widest mb-4">
                    <?= esc($hero['subtitle']) ?>
                </span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            ...
            <?php foreach ($projects as $index => $p): ?>
                <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all group flex flex-col h-full"
                    data-aos="fade-up" data-aos-delay="<?= $index * 100 ?>">
                    <div class="flex justify-between items-start mb-4">
                        <div
                            class="p-2 bg-navy/5 rounded-lg text-navy group-hover:bg-navy group-hover:text-white transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="<?= $icons[$p['icon']] ?? $icons['office-building'] ?>" />
                            </svg>
                        </div>
                        <span
                            class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100 uppercase tracking-wider">
                            <?= esc($p['badge']) ?>
                        </span>
                    </div>
                    ...
                    <div class="space-y-3 text-sm flex-grow">
                        <div>
                            <strong
                                class="text-slate-700 block mb-0.5 uppercase text-[10px] tracking-widest font-bold opacity-60">Problem:</strong>
                            <p class="text-slate-500 leading-relaxed"><?= esc($p['problem']) ?></p>
                        </div>
                        <div>
                            <strong
                                class="text-slate-700 block mb-0.5 uppercase text-[10px] tracking-widest font-bold opacity-60">Solution:</strong>
                            <p class="text-slate-500 leading-relaxed"><?= esc($p['solution']) ?></p>
                        </div>
                    </div>
                    <div class="mt-5 pt-4 border-t border-slate-100">
                        <strong
                            class="text-navy text-sm block mb-1 uppercase text-[10px] tracking-widest font-bold opacity-60">Result:</strong>
