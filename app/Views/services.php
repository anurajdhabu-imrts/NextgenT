...
<section class="relative w-full overflow-hidden bg-navy">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24">
        <div class="flex items-center justify-center text-center" data-aos="fade-up">
            <div class="max-w-3xl">
                <span
                    class="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-xs font-medium tracking-wide text-white/90 mb-5">
                    <?= esc($hero['subtitle']) ?>
                </span>
...
<section id="core-services" class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12" data-aos="fade-up">
            <span
                class="text-teal-accent font-bold tracking-[0.2em] uppercase text-[10px] bg-teal-50 px-3 py-1 rounded-full"><?= esc($core_header['subtitle']) ?></span>
            <h2 class="mt-4 text-3xl md:text-4xl font-bold text-navy leading-tight"><?= esc($core_header['title']) ?>
            </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-8">
            ...
            foreach ($services as $index => $s): ?>
                <div class="flex items-start gap-4 group" data-aos="fade-up" data-aos-delay="<?= $index * 50 ?>">
                    <div
                        class="shrink-0 w-10 h-10 rounded-lg bg-navy/5 flex items-center justify-center text-navy group-hover:bg-navy group-hover:text-white transition-all duration-300">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="<?= $iconMap[$s['icon']] ?? $s['icon'] ?>" />
                        </svg>
                    </div>
                    <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-10" data-aos="fade-up">
            <div>
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span
                        class="text-[10px] font-bold tracking-[0.2em] uppercase text-teal-accent bg-teal-50 px-2 py-1 rounded"><?= esc($hospital_header['subtitle']) ?></span>
                </div>
                <h2 class="text-3xl md:text-4xl font-bold text-navy text-base leading-relaxed">
                    <?= esc($hospital_header['title']) ?></h2>
            ...
            <a href="<?= base_url('hospital-sector') ?>"
                class="inline-flex items-center text-teal-accent font-bold text-sm shrink-0 group">
        ...
        <div class="space-y-4">
            ...
            <?php foreach ($services as $s): ?>
                <div class="bg-white rounded-xl border border-slate-100 p-5 flex flex-col sm:flex-row sm:items-start gap-4 hover:shadow-md hover:border-teal-accent/20 transition-all duration-300 group"
                    data-aos="fade-up">
                    <div
                        class="shrink-0 w-9 h-9 rounded-lg bg-teal-accent/10 flex items-center justify-center text-teal-accent">
