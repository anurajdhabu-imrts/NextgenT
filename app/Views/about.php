    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div data-aos="fade-right">
                <img src="<?= esc($journey['image']) ?>" alt="<?= esc($journey['title']) ?>"
                    class="rounded-2xl shadow-lg w-full h-80 object-cover" />
            </div>
            <div data-aos="fade-left">
                <h2 class="text-3xl font-bold text-navy mb-6">
                    <?= preg_replace('/\[(.*?)\]/', '<span class="text-teal-accent">$1</span>', $journey['title']) ?>
                </h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch">
            ...
            <div data-aos="fade-right" class="space-y-8">
                <div>
                    <h2 class="text-3xl font-bold text-navy mb-3">
                        <?= preg_replace('/\[(.*?)\]/', '<span class="text-teal-accent">$1</span>', $geographic_header['title']) ?>
                    </h2>
                ...
                <div class="space-y-6">
                    <?php foreach ($geographic_items as $index => $item): ?>
                        <div
                            class="pl-5 border-l-[3px] <?= $index === 0 ? 'border-teal-accent' : 'border-slate-200 transition-colors duration-300 hover:border-teal-accent group' ?>">
                            <h3 class="text-lg font-bold text-navy mb-1.5 flex items-center gap-2">
                                <?php if (strpos($item['icon'], 'uploads/') !== false): ?>
                                    <img src="<?= base_url($item['icon']) ?>" class="w-5 h-5 object-contain"
                                        alt="<?= esc($item['title']) ?>">
                                <?php else: ?>
                                    <svg class="w-5 h-5 <?= $index === 0 ? 'text-teal-accent' : 'text-slate-400 group-hover:text-teal-accent transition-colors' ?>"
            ...
            <div data-aos="fade-left" class="h-full flex items-stretch">
                <div class="w-full h-full bg-[#18113c] rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    <img src="<?= base_url($geographic_header['image']) ?>"
                        alt="<?= esc($geographic_header['title']) ?>"
...
<section class="py-16 bg-slate-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-3xl font-bold text-navy mb-4" data-aos="fade-up">
            <?= preg_replace('/\[(.*?)\]/', '<span class="text-teal-accent">$1</span>', $leadership_header['title']) ?>
        </h2>
            ...
            <?php foreach ($team as $index => $member): ?>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 group" data-aos="fade-up"
                    ...
                    <div class="aspect-square rounded-xl overflow-hidden mb-4">
                        <img src="<?= esc($member['image']) ?>" alt="<?= esc($member['name']) ?>"
                            class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                    </div>
                    <h3 class="text-lg font-bold text-navy"><?= esc($member['name']) ?></h3>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-12" data-aos="fade-up">
            <h2 class="text-3xl font-bold text-navy">
                <?= preg_replace('/\[(.*?)\]/', '<span class="text-teal-accent">$1</span>', $advantage_header['title']) ?>
            </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <?php foreach ($advantages as $index => $adv): ?>
                <div class="p-6 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center text-center group hover:bg-navy transition-all duration-300"
                    ...
                    <h3 class="font-bold text-navy mb-2 group-hover:text-white"><?= esc($adv['title']) ?></h3>
